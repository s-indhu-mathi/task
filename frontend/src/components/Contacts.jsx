...
const handleSubmit=(e)=>{
        ...
        if(contacts.name && contacts.lastName && contacts.status){
          ...
          alert("Some details are missing..,Please fill it out.")
        }
        setContacts({})
    }
...
const handleSubmitEdit=(e,id)=>{
    e.preventDefault();    
    try{
      axios.put(`https://taiyo-ai-server.onrender.com/contacts/${id}`,{name:edit.name,lastName:edit.lastName,status:edit.status}).then(()=>alert("Contact Edited Successfully")).then(()=>getData().then((res)=>setContacts(res.data)))
    }
    catch(err){
      console.log(err)
    }       
...
const deleteContact=(id)=>{
    axios.delete(`https://taiyo-ai-server.onrender.com/contacts/${id}`).then((res)=>alert("Contact Deleted Successfully")).then(()=>getData().then((res)=>setContacts(res.data)))
}
useEffect(()=>{
...
const viewContact=()=>{
  ...
  return (
    <div id="contact_page">
        <Heading color={'white'} p={"10px 20px"} bg={"#007bff"}>Contact Page</Heading>
        <div id="contact_page_div">
            ...
            <Box padding={"30px"} margin={'auto'}  w={"79%"} boxShadow={"rgba(0, 0, 0, 0.24) 0px 3px 8px"} border={"1px solid gray"}>
               {flag?<></>:<Button marginTop={"20px"} onClick={onOpen} colorScheme="teal">Create Contact</Button>}
               <Box>
                {
                    flag?
                    ...
                    <br />
                    <label>Status:</label>
                    <input type="radio" id="status" name="status" value="active" onChange={handleChange}/>
                    <label>Active</label>
                    <input type="radio" id="status" name="status" value="inactive" onChange={handleChange}/>
                        ...
                        <div id="data_container">
                            {data.map((el)=>(
                              <div key={el.id}>
                                ...
                                <Popover >
                                   <PopoverTrigger  >
                                    <button onClick={viewContact} style={{backgroundColor:"#bbc1c7"}}>View</button>
                                   </PopoverTrigger>
                                   <PopoverContent backgroundColor={'#bbc1c7'} padding={'20px'} color='white' margin={'auto'}  >
                               ...
                               <PopoverContent backgroundColor={'#194d33'} padding={'10px'} color='white' margin={'auto'}   >
                                        ...
                                        <label>Inactive</label>
                                          ...
                                          <button
                                            ...
                                            style={{ color:"white",backgroundColor:"#3fba80"}}
                                            >Submit</button>
                                </PopoverContent>
                            </Popover>
                            <button onClick={()=>deleteContact(el.id)} style={{ color:"white",backgroundColor:"#f14668"}}>Delete</button>
                              </div>
                            ))} 
                        </div>
                    </Box>
